    ...
    public class ConfiguracionCuentaViewModel : INotifyPropertyChanged
    {
        ...
        private async Task OnGuardarAsync()
        {
            ...
            if (!string.IsNullOrWhiteSpace(PasswordNuevo))
            {
                ...
                {
                    await Application.Current!.MainPage!.DisplayAlert("Error", "Debes ingresar tu contrase�a actual para cambiarla", "OK");
                    return;
                }

                if (PasswordNuevo != PasswordConfirmar)
                {
                    await Application.Current!.MainPage!.DisplayAlert("Error", "Las contrase�as nuevas no coinciden", "OK");
                    return;
                }

                if (PasswordNuevo.Length < 6)
                {
                    await Application.Current!.MainPage!.DisplayAlert("Error", "La nueva contrase�a debe tener al menos 6 caracteres", "OK");
                    return;
                }
            }
            ...
            try
            {
                ...
                if (usuario == null)
                {
                    await Application.Current!.MainPage!.DisplayAlert("Error", "No hay sesi�n activa", "OK");
                    return;
                }
